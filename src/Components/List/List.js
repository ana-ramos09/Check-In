...
const List = (props) => {
    ...
    const onDelete = (index) => {
        // TODO: criar ação específica para delete
        dispatch(loadList({...cityDetail, points: pointsList.splice(index,1)}));
    }

    const loadFirestoreList = (listName, listLocation) => {
        firestore.collection('lists').doc(listName + ' - ' + listLocation).get()
            .then((resp) => {
    ...
    return (
        <div className='list' id={ props.listId }>
            <div className='list-header-container'>
                <p id={ props.titleId } value={ props.titleContent }>{props.listName}</p>
                <button className='list-close' id={ props.listClose } title='Close List'>X</button>
            </div>
            <div className='location-container'>
                <figure alt='Location Symbol'></figure>
                <p>{props.listLocation}</p>
                <button onClick= { addPoint } title='Add Item'>Add Item!</button>
            </div>
            ...
            <button className='list-save' id={ props.saveList } onClick= { callSaveList } title='Save List'>SAVE</button>
        </div>
    )
}
