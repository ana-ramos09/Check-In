...
export const createList = (name, location) => {
    firestore.collection('lists').doc(name + ' - ' + location).set({
        name: name,
        location: location,
        points: []
    })
...
export const deleteList = (name, location) => {
    firestore.collection('lists').doc(name + ' - ' + location).delete()
    .then(() => {
...
export const saveList = (name, location, tasks) => {
    firestore.collection('lists').doc(name + ' - ' + location).update({ 
        points: tasks
    })
    .then(() => {
